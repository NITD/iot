$('#ip-btn').click(function () {
	...
	var socket = io.connect('http://' + server, { query: 'type=app&appType=control' });
	...
	$('#add-device-info-page').on('click', '.device-post-btn', function () {
		...
		$.post('http://' + server + '/api/' + type + '/' + obj.id, obj)
		.done(function () {
	...
	$('#device-info-page-update-btn').on('click', '.device-put-btn', function () {
		...
		$.ajax({
			method: 'PUT',
			url: 'http:' + server + '/api/' + type + '/' + currentStatusPage.id,
			data: obj
		})
		.done(function () {
	...
	$('#clients-page').on('click', '.add-client-btn', function () {
		...
		$.post('http://' + server + '/api/' + type + '/' + id + '/client/' + clientType, { id: clientid })
		.done(function () {
			$.post('http://' + server + '/api/' + clientType + '/' + clientid + '/client/' + type, { id: id })
			.done(function () {
	...
	function deleteClient(type, id, ctype, cid) {
		if (type === 'lamp' || type === 'garagedoor' || type === 'fan') {
			...
			$.ajax({
				method: 'DELETE',
				url: 'http://' + server + '/api/' + type + '/' + id + '/client/' + ctype
			})
			.fail(function () {
				window.alert('An error occurred');
			})
			...
			$.ajax({
				method: 'DELETE',
				url: 'http://' + server + '/api/' + type + '/' + id + '/client/' + ctype + '/' + cid
			})
			.fail(function () {
				window.alert('An error occurred');
			})
	...
	function getDevices() {
		$.getJSON('http://' + server + '/api/devices')
		.done(function (data) {
			if (!data) {
				window.alert('An error occurred');
				return;
